...
class RunnerTests: XCTestCase {
  ...
  func testSecurityScopedAccessIsDeduplicatedAndBalanced() throws {
    let firstURL = URL(fileURLWithPath: "/first")
    let secondURL = URL(fileURLWithPath: "/second")
    let files = [
      JxlFileAccess(fileURL: firstURL, securityScopedURL: firstURL),
      JxlFileAccess(fileURL: firstURL, securityScopedURL: firstURL),
      JxlFileAccess(fileURL: secondURL, securityScopedURL: secondURL),
      JxlFileAccess(fileURL: URL(fileURLWithPath: "/plain"), securityScopedURL: nil),
    ]
    var events: [String] = []

    let value = try withJxlSecurityScopedAccess(
      files,
      startAccessing: {
        events.append("start:\($0.path)")
        return true
      },
      stopAccessing: { events.append("stop:\($0.path)") }
    ) {
  ...
  func testSecurityScopedAccessStopsEarlierURLsWhenStartingFails() {
    let firstURL = URL(fileURLWithPath: "/first")
    let secondURL = URL(fileURLWithPath: "/second")
    let files = [
      JxlFileAccess(fileURL: firstURL, securityScopedURL: firstURL),
    ...
    XCTAssertThrowsError(try withJxlSecurityScopedAccess(
      files,
      startAccessing: {
        events.append("start:\($0.path)")
        return $0 == firstURL
      ...
      XCTFail("The operation must not run without every requested scope")
    }) { error in
      let native = error as NSError
      XCTAssertEqual(native.domain, NSCocoaErrorDomain)
      XCTAssertEqual(native.code, NSFileReadNoPermissionError)
      XCTAssertEqual(native.userInfo[NSFilePathErrorKey] as? String, secondURL.path)
    }
    XCTAssertEqual(events, ["start:/first", "start:/second", "stop:/first"])
  }

  func testSecurityScopedAccessStopsURLsWhenTheOperationThrows() {
    let url = URL(fileURLWithPath: "/scoped")
    let expected = NSError(domain: "Test", code: 1)
    var stopped = false

    XCTAssertThrowsError(try withJxlSecurityScopedAccess(
      [JxlFileAccess(fileURL: url, securityScopedURL: url)],
      startAccessing: { _ in true },
      stopAccessing: { _ in stopped = true }
    ) {
      throw expected
    }) { error in
  ...
  func testSecurityScopedBookmarksKeepFileAccessAliveThroughConversions() throws {
    let directory = try temporaryDirectory()
    defer { try? FileManager.default.removeItem(at: directory) }
    let inputURL = directory.appendingPathComponent("input.jpg")
    let jxlURL = directory.appendingPathComponent("output.jxl")
    ...
    let jpegData = try fixtureData("2.jpg")
    try jpegData.write(to: inputURL)
    let inputBookmark = try inputURL.bookmarkData(
      options: .withSecurityScope,
      includingResourceValuesForKeys: nil,
      relativeTo: nil
    )
    let directoryBookmark = try directory.bookmarkData(
      options: .withSecurityScope,
      includingResourceValuesForKeys: nil,
      relativeTo: nil
    )
    let inputData = FlutterStandardTypedData(bytes: inputBookmark)
    let directoryData = FlutterStandardTypedData(bytes: directoryBookmark)

    XCTAssertNil(invoke(
      "jpegPathToJxl",
      arguments: [
        inputURL.path, jxlURL.path, 3, 0, 1, 0,
        ["securityScopedBookmarks": [inputData, directoryData]],
      ]
    ))
    XCTAssertNil(invoke(
      "jxlPathsToJpeg",
      arguments: [
        1, 0, jxlURL.path, restoredURL.path,
        ["securityScopedBookmarks": [directoryData, directoryData]],
      ]
    ))
  ...
  func testSecurityScopedBookmarkMustContainRequestedPath() throws {
    let directory = try temporaryDirectory()
    defer { try? FileManager.default.removeItem(at: directory) }
    let inputURL = directory.appendingPathComponent("input.jpg")
    let outsideURL = directory.deletingLastPathComponent()
      .appendingPathComponent("outside-\(UUID().uuidString).jxl")
    try fixtureData("2.jpg").write(to: inputURL)
    let directoryBookmark = try directory.bookmarkData(
      options: .withSecurityScope,
      includingResourceValuesForKeys: nil,
      relativeTo: nil
    )
    let bookmarkData = FlutterStandardTypedData(bytes: directoryBookmark)

    assertFlutterError(
      invoke(
        "jpegPathToJxl",
        arguments: [
          inputURL.path, outsideURL.path, 3, 0, 1, 0,
          ["securityScopedBookmarks": [bookmarkData, bookmarkData]],
        ]
      ),
  ...
  func testEveryInvalidArgumentBoundaryReturnsInvalidArguments() {
    ...
    let calls: [(String, Any?)] = [
      ...
      ("jpegPathToJxl", [
        "in.jpg", "out.jxl", 7, 0, 1, 0,
        ["securityScopedBookmarks": []],
      ]),
      ("jpegPathToJxl", [
        "in.jpg", "out.jxl", 7, 0, 1, 0,
        ["securityScopedBookmarks": [NSNull(), 1]],
      ]),
      ("jpegPathToJxl", [
        "in.jpg", "out.jxl", 7, 0, 1, 0,
        ["wrongKey": [NSNull(), NSNull()]],
      ]),
